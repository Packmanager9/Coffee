...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    class LineOP {
        ...
        intersects(line) {
            ...
            if (det === 0) {
                ...
                return (-0.199 < lm && lm < 1.199) && (-0.199 < gm && gm < 1.199);
            }
        }
        angle() {
    ...
    class Circle {
        ...
        draw() {
            ...
            canvas_context.strokeStyle = this.color
            canvas_context.beginPath();
                canvas_context.arc(this.x, this.y, this.radius*2, 0, (Math.PI * 2), true)
                canvas_context.fillStyle = this.color
                canvas_context.fill()
                canvas_context.stroke();
            }
    ...
    function setUp(canvas_pass, style = "#000000") {
        ...
        function continued_stimuli(e) {
            ...
            XS_engine = e.clientX - FLEX_engine.left;
            YS_engine = e.clientY - FLEX_engine.top;
            TIP_engine.xmom = XS_engine-TIP_engine.x
            TIP_engine.ymom = YS_engine-TIP_engine.y
            TIP_engine.x = XS_engine
            TIP_engine.y = YS_engine
            TIP_engine.body = TIP_engine
        }
    ...
    class SpringOP {
        ...
        // }
        draw() {
            this.beam.color = `rgba(${((this.anchor.ref.r)+(this.body.ref.r))/2}, ${((this.anchor.ref.g)+(this.body.ref.g))/2}, ${((this.anchor.ref.b)+(this.body.ref.b))/2}, 1)`
           this.beam.width = Math.max(this.anchor.radius*2, this.body.radius*2)*1.8
            this.beam.draw()
        }
        move() {
        }
    ...
    class Water {
        constructor(x, y) {
            this.body = new Circle(x, Math.max((y-(cup.drops.length*.7)), cup.center.y-cup.center.radius), 5, getRandomColor(), 3*Math.cos(cup.drops.length), 1, 1, 1)
            this.r = ((cup.drops.length*cup.drops.length*cup.drops.length)*.000007)+110
            this.g = ((cup.drops.length*cup.drops.length*cup.drops.length)*.000007)+55
            this.b = ((cup.drops.length*cup.drops.length*cup.drops.length)*.000007)-30
            this.gravrat = (400-cup.drops.length)/600
            this.body.drop = 1
            // if (this.body.x< 250) {
            //     this.r = 512
            //     this.g = 0
        ...
        move(){
            this.body.ymom += ((((positiony) * .45)*(Math.max(( Math.max(1, this.links.length+1) -this.links.length),.1)*.5))*gravity.value*this.gravrat)*.4
            this.body.xmom += ((((positionx) * .45)*(Math.max(( Math.max(1, this.links.length+1)-this.links.length),.1)*.5))*gravity.value*this.gravrat)*.4
            ...
            this.body.xmom*=speedtrap.value*.9
            this.body.ymom*=speedtrap.value*.9
        }
        draw() {
            this.body.draw()
        }
    ...
    class Handler{
        ...
        draw(){
            this.smallsize = this.center.radius-30
            this.smallbody = new Circle(this.center.x, this.center.y, this.smallsize, "#bbaaaa")
            this.bigbody = new Circle(this.center.x, this.center.y, this.bigsize, "white")
            this.bigbody.draw()
            this.smallbody.draw()
        }
    ...
    class Cup {
        constructor() {
            ...
            this.stick = new LineOP(this.center, TIP_engine, "Red", 17)
            this.angle = -.5
            this.anglesto = -.5
            this.drops = []
            this.score = 0
        ...
        shake(){
            for(let t = 0;t<this.drops.length;t++){
                ...
                for(let k = 0;k<this.drops.length;k++){
                    let link = new LineOP(this.drops[t].body, this.drops[k].body)
                    this.drops[t].linksx[k] = link
                }
            }
        }
        draw() {
                ...
                for (let k = 0; k < this.drops.length; k++) {
                    if (t != k) {
                        ...
                        if (this.drops[t].body.doesPerimeterTouch(this.drops[k].body)) {
                            ...
                            const distance =  this.drops[t].linksx[k].hypotenuse()   //((new Line(this.drops[k].body.x, this.drops[k].body.y, this.drops[t].body.x, this.drops[t].body.y, 1, "red")).hypotenuse()) - ((this.drops[t].body.radius * 2))
                            const angleRadians = this.drops[t].linksx[k].angle() //Math.atan2(this.drops[k].body.y - this.drops[t].body.y, this.drops[k].body.x - this.drops[t].body.x);
                            ...
                            if (!this.drops[t].links.includes(k) && !this.drops[k].links.includes(t) && this.drops[t].links.length < (linklength.value) && this.drops[k].links.length <  (linklength.value) ) {
                                let link = new SpringOP(this.drops[t].body, this.drops[k].body, lengthslider.value, `rgba(${((this.drops[t].r)+(this.drops[k].r))/2}, ${((this.drops[t].g)+(this.drops[k].g))/2}, ${((this.drops[t].b)+(this.drops[k].b))/2}, 1)`)
            ...
            // }
            this.stick.draw()
            this.stick.object.x -= TIP_engine.xmom *.05
            TIP_engine.xmom *=.05
            this.stick.object.y -= TIP_engine.ymom *.05
            TIP_engine.ymom *=.05
                for(let k = 0; k<this.links.length;k++){
                    if(this.stick.intersects(this.links[k].beam)){
                        if(this.links[k].stirtime <=0){
                            this.links[k].anchor.xmom += Math.cos(this.stick.angle()+(Math.PI*.5))*25
                            this.links[k].anchor.ymom += Math.sin(this.stick.angle()+(Math.PI*.5))*25
                            // this.links[k].body.xmom -= Math.cos(this.stick.angle()+(Math.PI*.5))*15
                            // this.links[k].body.ymom -= Math.sin(this.stick.angle()+(Math.PI*.5))*15
                        }
                    }
                }
                ...
                this.rgb.b = 0
                for(let t = 0;t<this.drops.length;t++){
                    this.rgb.r += this.drops[t].r/this.drops.length
                    this.rgb.g += this.drops[t].g/this.drops.length
                    this.rgb.b += this.drops[t].b/this.drops.length
                }
            this.mixup = 0
            for(let t = 0;t<this.drops.length;t++){
                this.mixup += (this.drops[t].r -this.rgb.r)*(this.drops[t].r -this.rgb.r)
                this.mixup += (this.drops[t].g -this.rgb.g)*(this.drops[t].g -this.rgb.g)
                this.mixup += (this.drops[t].b -this.rgb.b)*(this.drops[t].b -this.rgb.b)
            }

            canvas_context.font = "30px arial"
            canvas_context.fillStyle = "white"
            canvas_context.fillText("Score: "+this.score,10,70)

            if(this.drops.length == 330){
                canvas_context.font = "30px arial"
                canvas_context.fillStyle = "white"
                canvas_context.fillText("Mixing Left: " + Math.round((this.mixup/1000)-100),10,30)
                if(this.mixup < 100000){
                    this.drops = []
                    this.score++
                }
            }
    ...
    let soulsds = new Audio()
    soulsds.src = "soulsds.mp3"
    soulsds.volume = .1
    ...
    function main() {
        ...
        if(pause == -1){
            ...
            if(cup.drops.length < drops.value &&((count%4 == 0 || (count%(Math.ceil(4-(cup.drops.length/140))) == 0)))){
                let drop = new Water(cup.center.x, cup.center.y)
                count++
                // if(Math.random()<.5){dropw.
                //     drop.r = 255
                // }else{
                //     drop.r = 0
                // }
